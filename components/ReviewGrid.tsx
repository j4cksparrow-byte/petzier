      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex items-end justify-between mb-12">
          ...
          <div className="hidden md:flex flex-col items-end">
            <span className="font-mono text-4xl font-semibold text-[#4A5842]">4.9</span>
            <StarRating rating={5} />
            <span className="font-mono text-xs text-[#22211E]/50 mt-1 tracking-wide">
              From 847 reviews
            </span>
          </div>
        </div>
        ...
        <div ref={ref} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviews.map((review, i) => (
            <article
              key={i}
              className="reveal border border-[#B5A48C] p-6 flex flex-col gap-4 hover:border-[#4A5842] transition-colors duration-300"
            >
              {/* Top row */}
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  {/* Avatar */}
                  <div className="w-9 h-9 bg-[#4A5842] flex items-center justify-center flex-shrink-0">
                    <span className="font-mono text-xs text-[#EDE8DE] font-semibold">
                      {review.initials}
                    </span>
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-[#22211E]">{review.name}</p>
                    <p className="font-mono text-[0.6rem] tracking-[0.08em] text-[#22211E]/50 uppercase">
                      {review.location}
                    </p>
                  </div>
                </div>
              ...
              {/* Review text */}
              <blockquote className="text-sm text-[#22211E]/75 leading-relaxed flex-1">
                &ldquo;{review.text}&rdquo;
              </blockquote>

              {/* Product + Pet tag */}
              <div className="flex items-center justify-between pt-3 border-t border-[#B5A48C]/50">
                <span className="font-mono text-[0.6rem] tracking-[0.08em] uppercase text-[#22211E]/40">
                  {review.product}
                </span>
                <span className="font-mono text-[0.6rem] tracking-[0.08em] uppercase text-[#4A5842]">
                  Pet: {review.petName}
                </span>
              </div>
            </article>
